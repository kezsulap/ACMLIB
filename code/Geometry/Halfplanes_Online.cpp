...
struct line {
	...
	line(ll a_ = 0, ll b_ = 0, ll c_ = 0): a(a_), b(b_), c(c_)
		{} // <= 10^9
	line (P const &A, P const &B): a(A.Y-B.Y), b(B.X-A.X),
		c(A.X*B.Y-A.Y*B.X) {} //pts <= 10^6
...
inline bool pokr(line a, line b)
	{return rown(a,b) && a.a*b.c == b.a*a.c && a.b*b.c == b.b*a.c;}
...
struct przeciecie_polplaszczyzn {
	...
	int type() {
		...
		if(siz(S) <= 4){
			...
			if (siz(res) == 2 && rown(res[0], res[1]) && weaker(res[0], -res[1])<0)
				return 0; 
			for (int i = 0; i < siz(res); ++i) for (int j = 0; j < i; ++j)
				if(pokr(res[i], res[j])) {
